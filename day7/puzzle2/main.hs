...
calculate2 :: [ByteString] -> Word16 -> Calc Word16
calculate2 s n = do
    parse s
    modMap "b" $ Wire { input = Input { op = NOP, operands = [BS.pack (show n)] }, output = Just n }
    signal "a"
...
main = do
    ...
    x <- BS.readFile path
    let l = BS.lines x
        a = evalState (calculate l) $ CalcState { wm = Map.empty }
        b = evalState (calculate2 l a) $ CalcState { wm = Map.empty }
    putStrLn $ show b
