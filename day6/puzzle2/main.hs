...
editLights g c = [
    Light {
        ...
        status = if (x l) >= (x1 c) && (x l) <= (x2 c) && (y l) >= (y1 c) && (y l) <= (y2 c)
                     then case (op c) of {
                              On -> (status l) + 1;
                              Off -> if (status l) == 0 then 0 else (status l) - 1;
                              Toggle -> (status l) + 2
                          }
                     else status l
    } | l <- g ]

countTotalLights :: Grid -> Int
countTotalLights g = foldl' (+) 0 $ [ status l | l <- g ]
...
initLights :: Int -> Grid
initLights s = [ Light {x = (mod g s), y = (quot g s), status = 0} | g <- [0..(s * s)]]
...
main = do
    ...
    putStrLn $ show $ countTotalLights e
